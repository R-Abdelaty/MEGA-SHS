...
def run_schedule_repair(
    ...
    explicitly true after the strict run fails and the user authorizes that
    reported relaxation. The limits apply only to alternative candidates and
    output detail pages; there is no affected-session total limit. Follow
    ``next_result_offset`` until the final page when all detailed outcomes are
    required.
    ...
    ``approve_repair``. The bounded search does not prove global optimality.
    """
    request = {
    ...
    response: dict[str, Any] = {
        ...
        "constraint_policy": {
            "priority_order": "Exam/Quiz > Lecture > Tutorial > Laboratory",
            "unaffected_sessions_frozen": True,
